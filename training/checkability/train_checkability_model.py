"""Config-driven multilingual checkability training (5-class)."""
...
def _check_data_files(cfg: Dict[str, Any]) -> None:
    ...
    if missing:
        raise FileNotFoundError("Missing checkability dataset files:\n" + "\n".join(missing))
...
def _save_loss_plot(trainer: Trainer, output_dir: Path) -> None:
    ...
    plt.xlabel("Step")
    plt.ylabel("Loss")
    plt.title("Checkability Training Loss")
    plt.legend()
    plt.tight_layout()
...
def _save_confusion_matrix(
    ...
    cm = confusion_matrix(refs, preds, labels=ids)
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=labels)
    fig, ax = plt.subplots(figsize=(8, 7))
    disp.plot(ax=ax, xticks_rotation=25, colorbar=False)
    plt.title(f"Checkability Confusion Matrix ({split_name})")
    plt.tight_layout()
...
def _build_metrics_fn():
    def compute_metrics(eval_pred):
        ...
        macro_f1 = f1_score(refs, preds, average="macro", zero_division=0)
        weighted_f1 = f1_score(refs, preds, average="weighted", zero_division=0)
...
def run_training(config_path: str) -> None:
    ...
    labels = list(cfg.get("labels", DEFAULT_LABELS))
    ...
    text_col = dcfg.get("text_column", "text")
    y_col = dcfg.get("label_column", "label_id")

    model_name = str(cfg["model_name"])
    model_init_path = str(cfg.get("model_init_path", model_name))
    tokenizer = AutoTokenizer.from_pretrained(model_init_path, use_fast=False)

    def preprocess(batch):
        enc = tokenizer(batch[text_col], truncation=True, max_length=int(tcfg["max_seq_length"]))
        raw_labels = batch[y_col]
        if raw_labels and isinstance(raw_labels[0], str):
            enc["labels"] = [label2id[str(x)] for x in raw_labels]
        else:
            enc["labels"] = [int(x) for x in raw_labels]
        return enc
    ...
    refs = pred.label_ids
    _save_confusion_matrix(refs, preds, output_dir, "test", labels)
    ...
    with (records / "checkability_training_runs.jsonl").open("a", encoding="utf-8") as f:
        f.write(json.dumps(run_summary, ensure_ascii=False) + "\n")

    print("Checkability training complete")
